...
export const GridSupport = ({ data }: { data: [] }) => {
        ...
        <div className="flex flex-col gap-1 2xl:flex-row 2xl:gap-0 items-center">
          <Button isLink={true} className="hover:no-underline">
            ...
            <Link
              ...
              state={{ token: row.referenceToken, taskMainId: row.taskItemId }}
            >
              Görüntüle
            </Link>
          </Button>
    ...
    if (data.length) {
      const tableData: DataType[] = data.map((task: any, index: number) => {
        return {
          ...
          badge: "primary",
          referenceToken: task.referanceToken,
          taskItemId: task.taskItems[0].id,
        };
      });
  ...
  return (
    <>
      ...
      />
      <div className="outlet w-full h-full mt-4 pb-8 pt-4">
